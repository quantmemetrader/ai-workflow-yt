...
export function IdeasPanel({ zh, initial, canStart, canResearch = true }: { zh: boolean; initial: Idea[]; canStart: boolean; canResearch?: boolean }) {
  ...
  return (
    <section style={{ borderRadius: 16, border: "1px solid transparent", background: "linear-gradient(#ffffff, #ffffff) padding-box, linear-gradient(135deg, #d5e7fb, #dcd6fb 55%, #d5e7fb) border-box", padding: "14px 16px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10, minWidth: 0 }}>
        <AgentIcon agent="research" size={24} radius={7} />
        ...
        {canResearch ? (
          <Link prefetch={false} href="/research/backlog" style={{ fontSize: 12, color: "#525252", textDecoration: "none", whiteSpace: "nowrap" }}>
            {t("查看全部 →", "See all →")}
          </Link>
        ) : null}
      </div>
      ...
      >
        ...
        />
        <button type="submit" disabled={busy} style={{ height: 32, padding: "0 13px", borderRadius: 9, border: 0, background: "#171717", color: "#fff", fontFamily: "inherit", fontSize: 12.5, cursor: busy ? "default" : "pointer", display: "inline-flex", alignItems: "center", gap: 6, flexShrink: 0, opacity: busy ? 0.7 : 1 }}>
          <Icon name="spark" size={13} /> {busy ? t("生成中…", "Working…") : visible.length ? t("再出一批", "New batch") : t("生成选题", "Generate ideas")}
        </button>
      </form>
      ...
      {!visible.length && !busy ? (
        <div style={{ marginTop: 12, padding: "12px 13px", borderRadius: 11, background: "#fafbfd", border: "1px dashed #dfe6f1" }}>
      ...
      {visible.length ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 12 }}>
          {visible.map((idea, i) => {
            ...
            return (
              <div key={idea.id} style={{ borderTop: i ? "1px solid #f0f0f0" : "none", paddingTop: i ? 10 : 0 }}>
                <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                  <span style={{ fontSize: 15, fontWeight: 600, lineHeight: 1.4, color: "#171717" }}>{idea.title}</span>
                  {idea.strength ? (
                    <span title={t("研究员觉得有多强", "How strong the researcher thinks it is")} style={{ fontSize: 10.5, color: "#c2410c", letterSpacing: 1, flexShrink: 0 }}>
                      {"●".repeat(idea.strength)}
                      {"○".repeat(Math.max(0, 5 - idea.strength))}
                    </span>
                  ) : null}
                  {idea.format ? <span style={{ fontSize: 11, color: "#7c7c7c", background: "#f3f3f1", borderRadius: 999, padding: "0 7px", lineHeight: "17px", flexShrink: 0 }}>{idea.format}</span> : null}
                </div>
                {idea.titles.length ? <div style={{ fontSize: 12, color: "#7c7c7c", marginTop: 3, lineHeight: 1.5 }}>{t("也可以叫：", "Or: ")}{idea.titles.join(" / ")}</div> : null}
                {idea.angle ? <div style={{ fontSize: 12.5, color: "#2b343d", marginTop: 5, lineHeight: 1.6 }}>{t(`角度：${idea.angle}`, `Angle: ${idea.angle}`)}</div> : null}
                {idea.why ? (
                  <div style={{ display: "flex", gap: 7, alignItems: "flex-start", marginTop: 5 }}>
                    <span style={{ flexShrink: 0, marginTop: 1 }}>
                      <AgentIcon agent="research" size={16} radius={5} />
                    </span>
                    <span style={{ fontSize: 12.5, color: "#525252", lineHeight: 1.6 }}>{idea.why}</span>
                  </div>
                ) : null}
                {idea.hook ? <div style={{ fontSize: 12, color: "#7c7c7c", marginTop: 4, lineHeight: 1.55 }}>{t(`开头：「${idea.hook}」`, `Opening: “${idea.hook}”`)}</div> : null}
                ...
                {done ? (
                  ...
                  <div style={{ display: "flex", gap: 6, marginTop: 10, flexWrap: "wrap", alignItems: "center" }}>
                    {idea.status === "started" && projectId ? (
                      ...
                      <button type="button" disabled={working !== null} onClick={() => startOne(idea)} style={{ ...btn(true), opacity: working === idea.id ? 0.6 : 1 }}>
...
export function StartedNotice({ zh, title, projectId, scriptId, writing, existed, onStay }: { zh: boolean; title: string; projectId: string; scriptId: string | null; writing: boolean; existed: boolean; onStay: () => void }) {
  const t = (a: string, b: string) => (zh ? a : b);
  return (
    <div role="status" style={{ marginTop: 10, padding: "10px 12px", borderRadius: 11, background: "#f4fbf8", border: "1px solid #c3e6e0", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
