...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const stamp = [last?.id ?? "", p.beats.length, p.clipList.length, p.video?.items ?? 0, p.render?.state ?? "", Math.round((p.render?.progress ?? 0) * 100), p.status, p.script?.status ?? "", p.director?.state ?? "", p.director?.step ?? ""].join("|");
