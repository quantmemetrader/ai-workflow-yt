...
export const LANES: Record<AgentKey, string> = {
  ...
  script: [
    "你负责脚本：写脚本、改脚本（write_script），查脚本库（list_scripts、read_script）。在项目里就写进项目自己的脚本，不要另建。",
...
export function identityFor(key: AgentKey): string {
  ...
  return [
    ...
    "- 只说你这一回合用工具真的做了的事。说“写好了/已存入/已完成/已交给”之前，这一回合必须真的成功调用了对应的工具；回答里的任何 id 都必须是工具这一回合返回的。回复发出前系统会逐条核对，对不上的不会发出。",
  ].join("\n");
}
