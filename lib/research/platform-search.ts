...
import { bilibiliSearchVideos, xiaohongshuSearchNotes } from "@/lib/social/tikhub";
...
async function live(platform: SearchPlatform, q: string): Promise<HotRow[]> {
  ...
  /* B站 and 小红书 go through the same readers the beat feeds use
     (`lib/social/tikhub.ts`), so each endpoint is parsed in one place; the
     parameters are the ones this search always asked for. */
  if (platform === "bilibili") return bilibiliSearchVideos(q, { order: "totalrank", size: 12 });
  if (platform === "douyin") {
  ...
  return xiaohongshuSearchNotes(q, { sort: "popularity_descending" });
}
