...
export async function judgeHot(tenantId: string, platform: PlatformKey, rows: HotRow[], fetchedAt: number): Promise<Judged> {
  const key = `${tenantId}:${platform}:${fetchedAt}`;
  ...
  const prompt = [
    ...
    "输出 JSON 数组，每项 {\"n\": 序号, \"fit\": \"2到6个字的标签，如 可接RWA选题 / 观众问过 / 香港本地 / 形式可借\", \"why\": \"一句话依据\"}。没有就输出 []。只输出 JSON。",
    "",
    "# 频道数据",
    brief.text,
    "",
