...
## `pixel/01.svg` to `pixel/08.svg`

- Drawn for this app: 16x16 pixel people in the same letter grid, outline,
  skin tones and tinted tiles as the AI employees' faces
  (`lib/agents/pixel.ts`), so a person can match the employees if they like.
- Licence: original to this repository, under the same terms as the rest of it.
